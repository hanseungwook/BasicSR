...
def main():
    ...
    # HR train images
    opt['input_folder'] = '/disk_c/han/data/church_outdoor_train_imgs'
    opt['save_folder'] = '/disk_c/han/data/lsun_church_256x256/train/'
    opt['crop_size'] = 256
    extract_subimages(opt)

    # HR val images
    opt['input_folder'] = '/disk_c/han/data/church_outdoor_val_imgs'
    opt['save_folder'] = '/disk_c/han/data/lsun_church_256x256/val/'
    opt['crop_size'] = 256
    extract_subimages(opt)
...
def worker(path, opt):
    ...
    cropped_img = cv2.resize(img, (crop_size, crop_size), cv2.INTER_LINEAR)
    cropped_img = np.ascontiguousarray(cropped_img)

    cv2.imwrite(
        osp.join(opt['save_folder'],
                    f'{img_name}{extension}'), cropped_img,
